...
public class SteepestDescent {
	...
	public void run(int i, Polynomial P) {
		...
		for (int c = 0; c <= this.maxIter; c++) {
			...
			if (c != this.maxIter) {
				...
				double stepSize = this.lineSearch(P, this.bestPoint.get(i));
				...
				for (int j = 0; j < this.bestPoint.get(i).length; j++) { // update x for the next iteration
					...
					tempBestPoint[j] = this.bestPoint.get(i)[j] + (stepSize * cur_direction[j]);
	...
	public boolean getParamsUser() throws IOException {
		...
		// place holder for overriden methods
	...
	public void printStats() {
		...
		System.out.format("Average%13.3f%13.3f%18.3f\n", Stats.calcAverage(this.bestGradNorm),
				Stats.calcAverage(this.nIter), Stats.calcAverage(this.compTime));
		System.out.format("St Dev%14.3f%13.3f%18.3f\n", Stats.calcSD(this.bestGradNorm), Stats.calcSD(this.nIter),
				Stats.calcSD(this.compTime));
		System.out.format("Min%17.3f%13.0f%18.0f\n", Stats.calcMin(this.bestGradNorm), Stats.calcMin(this.nIter),
				Stats.calcMin(this.compTime));
		System.out.format("Max%17.3f%13.0f%18.0f\n", Stats.calcMax(this.bestGradNorm), Stats.calcMax(this.nIter),
				Stats.calcMax(this.compTime));
	}
	...
	public void printSingleResult(int i, boolean rowOnly) {
		...
		System.out.format("%8d%13.6f%13.6f%9d%17d", i + 1, this.bestObjVal[i], this.bestGradNorm[i], this.nIter[i],
				this.compTime[i]);
		System.out.print("   ");
	...
	public double[] getAverage() {
		...
		// get the averages for each array
		
		double[] array = { Stats.calcAverage(this.bestGradNorm), Stats.calcAverage(this.nIter),
				Stats.calcAverage(this.compTime) };

		return array;

	}

	public void printAverage(boolean rowOnly, String search) {
		...
		System.out.format("%-6s%14.3f%13.3f%18.3f\n", search, Stats.calcAverage(this.bestGradNorm),
				Stats.calcAverage(this.nIter), Stats.calcAverage(this.compTime));
